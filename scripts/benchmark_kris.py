"""Run the generic, auditable benchmark on the processed Kris catalog with automated charts and reporting.

Produces raw runs, fingerprints, paired comparisons and automatically generates
comparison charts and markdown reports into <output>_report.
"""

import argparse
import hashlib
import json
from collections import defaultdict
from pathlib import Path
import sys

if sys.platform == "win32":
    try:
        sys.stdout.reconfigure(encoding="utf-8")
        sys.stderr.reconfigure(encoding="utf-8")
    except AttributeError:
        pass

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from src.benchmark import benchmark
from src.models import write_json


def select_entries(catalog, orders=None, limit_per_size=0):
    entries = sorted(catalog, key=lambda row: (row["orders"], row["file"]))
    if orders:
        wanted = set(orders)
        entries = [row for row in entries if row["orders"] in wanted]
    if limit_per_size:
        by_size = defaultdict(list)
        for row in entries:
            by_size[row["orders"]].append(row)
        entries = [row for size in sorted(by_size) for row in by_size[size][:limit_per_size]]
    return entries


def main(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--catalog", type=Path, default=ROOT / "data/processed/kris_small/catalog.json",
                        help="Path to Kris catalog JSON (default: data/processed/kris_small/catalog.json)")
    parser.add_argument("--output", type=Path, default=ROOT / "results/kris_benchmark",
                        help="Output directory for raw benchmark runs (default: results/kris_benchmark)")
    parser.add_argument("--report-output", type=Path, default=None,
                        help="Output directory for charts and report (default: <output>_report)")
    parser.add_argument("--seconds", type=float, default=1.0,
                        help="Search budget in seconds per algorithm run (default: 1.0)")
    parser.add_argument("--search-seeds", type=int, nargs="+", default=[21, 22, 23],
                        help="Random search seeds (default: 21 22 23)")
    parser.add_argument("--orders", type=int, nargs="+", default=None,
                        help="Restrict to selected order-count strata")
    parser.add_argument("--limit-per-size", type=int, default=0,
                        help="Keep only the first N files per order-count stratum; 0 means all")
    parser.add_argument("--no-report", action="store_true",
                        help="Skip generating comparison charts and report")
    args = parser.parse_args(argv)
    if args.seconds <= 0 or args.limit_per_size < 0 or not args.search_seeds:
        parser.error("seconds must be positive; limit-per-size must be nonnegative; search-seeds nonempty")

    catalog_path = args.catalog.resolve()
    if not catalog_path.is_file():
        raise SystemExit(f"Catalog file not found: {catalog_path}")

    catalog = json.loads(catalog_path.read_text(encoding="utf-8"))["instances"]
    entries = select_entries(catalog, args.orders, args.limit_per_size)
    if not entries:
        raise SystemExit("No compatible Kris instances selected")

    output = args.output.resolve()
    if output.exists():
        raise SystemExit(f"Thư mục output đã tồn tại: {output}\nVui lòng chỉ định một thư mục mới (ví dụ: {output}_01).")

    report_output = args.report_output.resolve() if args.report_output else output.parent / f"{output.name}_report"

    config = {
        "instances": [str((ROOT / row["file"]).resolve()) for row in entries],
        "methods": ["B0", "B2", "LNS", "ALNS", "VNS"],
        "references": ["B0", "B2", "LNS", "VNS"],
        "search_seeds": args.search_seeds,
        "search": {"seconds": args.seconds, "iterations": 100000, "segment": 10},
        "weights": [1 / 3, 1 / 3, 1 / 3],
    }

    print(f"🚀 [1/2] Bắt đầu chạy Benchmark Kris catalog (B0, B2, LNS, ALNS, VNS)...")
    print(f"   - Catalog: {catalog_path}")
    print(f"   - Số lượng instance: {len(entries)}")
    print(f"   - Thư mục lưu dữ liệu thô: {output}")

    rows = benchmark(config, output, progress=lambda row: print(
        f"   [{row['instance']}] {row['method']:<4} seed={row['search_seed']} F={row['objective']:.5f} (quãng đường={row['distance']:.1f})", flush=True))

    source_selection = {
        "catalog": str(catalog_path),
        "catalog_sha256": hashlib.sha256(catalog_path.read_bytes()).hexdigest(),
        "selection_rule": "All catalog files, sorted by (order count, filename), unless --orders or --limit-per-size is supplied.",
        "entries": entries,
        "config": config,
        "runs": len(rows),
    }
    write_json(output / "source_selection.json", source_selection)
    print(f"\n✅ Đã hoàn thành {len(rows)} lượt chạy từ {len(entries)} Kris instance và lưu vào: {output}")

    if not args.no_report:
        from scripts.build_comparison_report import build_report
        print(f"\n📊 [2/2] Đang tự động kiểm định và vẽ biểu đồ so sánh...")
        build_report(output, report_output)
        print(f"🎉 Báo cáo và biểu đồ đã được lưu hoàn tất tại:")
        print(f"   📂 {report_output}")
        print(f"   ├── 📈 Biểu đồ so sánh: {report_output / 'charts'}")
        print(f"   ├── 📑 Bảng số liệu chi tiết: {report_output / 'comparison.csv'}")
        print(f"   └── 📝 Báo cáo tổng hợp: {report_output / 'REPORT.md'}\n")


if __name__ == "__main__":
    main()
